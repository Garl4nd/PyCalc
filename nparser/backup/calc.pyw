...
# TODO: \sqrt  -> odmocnitko -done
# TODO: vypinani plotovacich oken s vypnutim programu - done
# TODO: při appendování do paměti posunout pozici o +1
    ...
    def init_default_settings(self):
        self.default_settings={"design":{"dimensions":[300,235],"button_style": "background-color: None","display_style":"background-color: None",                                   "button_font_height_ratio": 0.3,"display_font_height_ratio": 0.4,"display_to_keyboard_percentage":18},
                                "parser":{"implicit_conversion":True,"implicit_ops":True,
                                        "fractions":True,"solve_w_complex":True,"solformat":True,"backend":"TkAgg"},
    ...
    def handle_plot(self,text,plot2d=None):
        if plot2d is None:
            if text.startswith("2"):
                ...
                plot2d=False
        args=("from","to","style","pden","grid","polar","cont","hold","anim")
        llim=None;ulim=None;style="";pden=None
        from_res,to_res,style_res,pden_res,grid_res,polar_res,cont_res,hold_res,anim_res=[list(self.parser.find_all(text,pat)) for pat in args]
        if len(from_res)>1:
        ...
        if len(hold_res)>1:
              raise ValueError("Please specify only a single 'hold' keyword")    
        if len(anim_res)>1:
              raise ValueError("Please specify only a single 'anim' keyword")    
        ...
        if found:
            ...
            hold_part=re.findall(f"hold(.*?){dlimit}",bounds)
            anim_part=re.findall(f"anim(.*?){dlimit}",bounds)
            #print(anim_part)
            if from_part:
                try:
        ...
        if plot2d:
            ...
            if not type(res) ==rp.Vector:
                ...
                if len(vars)==1 and next(iter(vars))=="t" and res.dim==2:
                    if ulim==None:
                        ulim=2*3.14
                    if not any(type(comp) is rp.Vector  for comp in res):
                        self.parser.plot(res,llim=llim,ulim=ulim,style=style,pden=pden,use_grid=use_grid,polar=polar,contours=contours,hold=hold,anim=anim)
                        return
            self.parser.plot(*res,llim=llim,ulim=ulim,style=style,pden=pden,use_grid=use_grid,polar=polar,contours=contours,hold=hold,anim=anim)
